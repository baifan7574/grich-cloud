...
import { buildReportDraft, saveReportDraft } from "../../_lib/reports.js";
...
export async function onRequestPost(context) {
    ...
    const draft = await buildReportDraft(env, order);
    await saveReportDraft(db, order.id, draft, "drafted");
    return jsonResponse({ status: "drafted", request_id: order.id, product_tier: order.product_tier, draft });
}
