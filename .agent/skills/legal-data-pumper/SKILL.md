...
1. 自动化运行标准
零容忍失败：GitHub Actions 出现红叉 即视为【任务失败】。禁止报告“脚本逻辑无误”等废话。

环境预检：脚本运行前，必须核查 SUPABASE_URL 和 SUPABASE_KEY 是否已在 GitHub Secrets 中配置。

2. 双脚本监控逻辑 (Courts & Law Firms)
法院脚本：必须输出当日抓取的具体案号数量。若数量为 0，必须自动检查法院官网是否改版。
...
脚本运行的第一步必须是检测 SUPABASE_URL 和 SUPABASE_KEY 是否存在。

如果不存在，必须立刻报错并停止，严禁谎称“环境自适应”。

日志穿透核验：

如果运行失败（红叉），AI 必须自动读取 GitHub Action 的 Step Logs。

必须找出是哪一行 Python 代码报错（如 ModuleNotFoundError 或 AuthError）。
